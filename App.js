...
export default class App extends Component {
      ...
      if(type==='work'){
          if(unit==='seconds'){
              ...
              if(input === ''){
                  ...
                  this.setState({minutes_Counter: input.length == 1 ? '0' + input : input,
          ...
          if(unit==='seconds'){
              if(input === ''){
                  this.setState({storedBreakSeconds: '00'})
              }
              else{
  ...
  Start = () => {
    let timer = setInterval(() => {
      ...
      if(this.state.seconds_Counter === '01' && this.state.minutes_Counter === '00'){
        ...
        if(this.state.seconds_Counter == '00' && this.state.minutes_Counter == '00' && this.state.type == 'work') {
            this.handleBreak()
        }
  ...
  handleBreak = () => {
      ...
      this.setState({
          minutes_Counter:'01',
          seconds_Counter:this.state.storedBreakSeconds,
  ...
  Clear = () => {
    ...
    this.timePicker('minutes','work',this.state.storedWorkMinutes)
      this.setState({titleText:'Work Time'})
  }
  ...
  render() {
    return (
        <View style={styles.container}>
          ...
          <Text style={{fontSize: 56, paddingBottom:10}}>{this.state.minutes_Counter}:{this.state.seconds_Counter}</Text>
