    ...
    if (type === 'work') {
      if (unit === 'seconds') {
        if (input === '') {
          ...
          if (Number(input) > 59) {
            ...
            this.setState({
              seconds_Counter: input.length == 1 ? '0' + input : input,
      ...
      if (unit === 'seconds') {
        if (input === '') {
          ...
          if (Number(input) > 59) {
            this.setState({
              storedBreakSeconds: '59',
              secBreak: '59',
              secWarn: true,
            });
          } else {
            this.setState({
              storedBreakSeconds: input.length == 1 ? '0' + input : input,
        ...
        if (input === '') {
          ...
          this.setState({
            storedBreakMinutes: input.length == 1 ? '0' + input : input,
  ...
  render() {
    return (
      <View style={styles.container}>
        <Text style={{fontSize: 46, marginBottom: 150}}>Pomodoro Timer</Text>
        <Text style={{fontSize: 60, fontWeight: 'bold'}}>
          {this.state.titleText}
        </Text>
        {this.state.showWork && (
          <Text style={{fontSize: 56, paddingBottom: 10}}>
            {this.state.minutes_Counter}:{this.state.seconds_Counter}
          </Text>
        )}
        {this.state.showBreak && (
          <Text style={{fontSize: 56, paddingBottom: 10}}>
            {this.state.minBreak}:{this.state.secBreak}
          </Text>
        )}
        <View style={styles.activeTimePicker}>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <UserTextInput
              ...
              onChangeText={text => this.timePicker('minutes', 'work', text)}
            />
          </View>
          <View style={{flexDirection: 'row'}}>
            ...
            <UserTextInput
              style={styles.timeBox}
              onChangeText={text => this.timePicker('seconds', 'work', text)}
            />
          </View>
        </View>

        <View
          style={[styles.activeTimePicker, {marginTop: 10, paddingBottom: 30}]}>
          <View>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <UserTextInput
              style={styles.timeBox}
              onChangeText={text => this.timePicker('minutes', 'break', text)}
            />
          </View>
          <View style={{flexDirection: 'row'}}>
            <Text style={styles.selectText}>Seconds: </Text>
            <UserTextInput
              style={styles.timeBox}
              onChangeText={text => this.timePicker('seconds', 'break', text)}
            />
          </View>
        </View>

        {this.state.secWarn && (
          <Text style={{marginTop: -20, marginBottom: 20, marginLeft: 150}}>
            Seconds will be set as '59'
          </Text>
        )}

        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          <TouchableOpacity onPress={this.Start} style={styles.buttons}>
          ...
          <TouchableOpacity onPress={this.Clear} style={styles.buttons}>
